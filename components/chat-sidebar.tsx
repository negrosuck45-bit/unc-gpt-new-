...
import {
  PanelLeftClose, PanelLeft, Edit2, MoreHorizontal,
} from "lucide-react"
import { motion, AnimatePresence } from "framer-motion"
import { ProjectsDialog } from "./projects-dialog"
import { ChatHistoryPanel } from "./chat-history-panel"
import { AuthDialog } from "./auth-dialog"
import { AccountSettings, Avatar } from "./account-settings"
...
export function ChatSidebar({ isOpen, onToggle, onChatSelect, onModeChange, isMobile = false }: ChatSidebarProps) {
  const { chats, currentChatId, currentProjectId, setCurrentChat, setCurrentProject, createNewChat, deleteChat, updateChatProject, appendToProjectMemory, updateChatTitle } = useChatStore()
  ...
  const groupedChats = useMemo(() => {
    ...
    const groups: Record<string, typeof chats> = { today: [], yesterday: [], previous7Days: [], previous30Days: [], older: [] }
    filteredChats.forEach(chat => {
      const d = new Date(chat.createdAt)
      if (d >= today) groups.today.push(chat)
      else if (d >= yesterday) groups.yesterday.push(chat)
      else if (d >= last7) groups.previous7Days.push(chat)
      else if (d >= last30) groups.previous30Days.push(chat)
  ...
  const handleNew = (type: "text" | "voice") => {
    const existingEmpty = chats.find(c => c.type === type && c.messages.length === 0 && c.title === "New Chat")
    if (existingEmpty) { setCurrentChat(existingEmpty.id); onModeChange(type); onChatSelect(existingEmpty.id, type); return }
  ...
  // ── Profile button — shown at the bottom of the sidebar ──────────────────
  const ProfileButton = ({ collapsed = false }: { collapsed?: boolean }) => {
    ...
    if (!profile) {
      return collapsed ? (
        <button onClick={() => setAuthOpen(true)} title="Sign in" className="h-9 w-9 rounded-xl flex items-center justify-center hover:bg-accent/50 transition-colors">
          <div className="w-6 h-6 rounded-full bg-muted-foreground/30 flex items-center justify-center text-[10px] font-bold">?</div>
        </button>
      ) : (
        <button onClick={() => setAuthOpen(true)} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-accent/50 transition-colors text-left">
    ...
    return collapsed ? (
      <button onClick={() => setSettingsOpen(true)} title={profile.name ?? 'Account'} className="h-9 w-9 rounded-xl flex items-center justify-center hover:bg-accent/50 transition-colors">
        <Avatar profile={profile} size={28} />
      </button>
    ) : (
      <button onClick={() => setSettingsOpen(true)} className="w-full flex items-center gap-2.5 px-3 py-2 rounded-xl hover:bg-accent/50 transition-colors text-left group">
        <Avatar profile={profile} size={28} />
        <div className="flex-1 min-w-0">
  ...
  if (!isOpen && !isMobile) {
    return (
      ...
      >
        <button onClick={onToggle} title="Open sidebar" className="group relative h-10 w-10 rounded-xl flex items-center justify-center hover:bg-accent/50 transition-colors mb-1">
        ...
        <RailButton title="Join our Discord" onClick={() => window.open(DISCORD_URL, "_blank")}><DiscordIcon className="h-4 w-4" /></RailButton>
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="p-4 flex items-center justify-between shrink-0">
              ...
              <button onClick={onToggle} title="Close sidebar" className="h-8 w-8 flex items-center justify-center rounded-md hover:bg-accent/50 transition-colors">
            ...
            <div className="flex-1 overflow-y-auto px-2 pt-4 min-h-0">
              ...
              ].map(({ label, chats: g }) => g.length > 0 && (
                <ChatGroup key={label} label={label} chats={g}
                  currentChatId={currentChatId} hoveredChatId={hoveredChatId} renamingChatId={renamingChatId}
                  renamingText={renamingText} setRenamingText={setRenamingText} setHoveredChatId={setHoveredChatId}
                  handleSelect={handleSelect} handleDelete={handleDelete} handleRenameStart={handleRenameStart}
                  handleRenameSave={handleRenameSave} setRenamingChatId={setRenamingChatId} />
              ))}
              {filteredChats.length === 0 && (
                <div className="text-center py-8 text-muted-foreground text-sm">{searchQuery ? "No chats found" : "No chats yet"}</div>
              )}
            </div>

            {/* Bottom — Discord + user profile */}
      ...
      {historyOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-900/50 backdrop-blur-sm">
          <div className="w-full max-w-lg h-[600px] shadow-2xl">
            <ChatHistoryPanel onClose={() => setHistoryOpen(false)} onSelectChat={id => { const chat = chats.find(c => c.id === id); if (chat) handleSelect(id, chat.type) }} />
          </div>
        </div>
      )}
...
function NavItem({ icon, label, onClick, active }: { icon: React.ReactNode; label: string; onClick: () => void; active?: boolean }) {
  return (
    <button onClick={onClick} className={cn("w-full flex items-center gap-3 px-3 py-2 rounded-lg text-sm transition-all duration-200", active ? "bg-accent text-accent-foreground" : "text-sidebar-foreground hover:bg-accent/50")}>
...
// ── ChatGroup ─────────────────────────────────────────────────────────────────
function ChatGroup({ label, chats, currentChatId, hoveredChatId, renamingChatId, renamingText, setRenamingText, setHoveredChatId, handleSelect, handleDelete, handleRenameStart, handleRenameSave, setRenamingChatId }: any) {
            ...
            {renamingChatId === chat.id ? (
              <div className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-accent/30 border border-accent/50">
                <input autoFocus type="text" value={renamingText} onChange={e => setRenamingText(e.target.value)}
                  ...
                  onBlur={() => handleRenameSave(chat.id)} className="flex-1 bg-transparent border-none text-sm outline-none" placeholder="Rename..." />
              </div>
            ) : (
              <div onClick={() => handleSelect(chat.id, chat.type)} onMouseEnter={() => setHoveredChatId(chat.id)} onMouseLeave={() => setHoveredChatId(null)}
                className={cn("group flex items-center gap-2 px-2 py-1.5 rounded-lg cursor-pointer text-sm transition-all duration-200", currentChatId === chat.id ? "bg-accent text-accent-foreground" : "hover:bg-accent/50")}>
                <span className="flex-1 truncate">{chat.title || "Untitled"}</span>
                {hoveredChatId === chat.id && (
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <button onClick={e => e.stopPropagation()} className="p-1 hover:bg-background rounded-md transition-colors"><MoreHorizontal className="h-4 w-4" /></button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent align="end" className="w-40">
                      <DropdownMenuItem onClick={e => { e.stopPropagation(); handleRenameStart(e, chat.id, chat.title) }}><Edit2 className="h-4 w-4 mr-2" />Rename</DropdownMenuItem>
                      <DropdownMenuItem onClick={e => handleDelete(e, chat.id)} className="text-destructive focus:text-destructive"><Trash2 className="h-4 w-4 mr-2" />Delete</DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                )}
              </div>
...
function RailButton({ title, onClick, children, active }: { title: string; onClick: () => void; children: React.ReactNode; active?: boolean }) {
  return (
    <button onClick={onClick} title={title} aria-label={title}
      className={cn("h-9 w-9 rounded-xl flex items-center justify-center text-sidebar-foreground/70 hover:text-sidebar-foreground hover:bg-accent/50 transition-all duration-200", active && "bg-accent text-accent-foreground")}>
      {children}
    </button>
  )
}