          ...
          <AnimatePresence mode="wait">
            ...
            >
                ...
                <div className="space-y-5">
                  ...
                  <div className="rounded-[22px] border border-border bg-card p-4 shadow-sm">
                    ...
                    <label className="group flex min-h-24 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-emerald-400/25 bg-emerald-400/[0.035] px-4 text-center transition hover:border-emerald-400/50 hover:bg-emerald-400/[0.07]">
                      ...
                      <input type="file" accept="image/*,video/*" className="hidden" onChange={async (event) => { const file = event.target.files?.[0]; event.currentTarget.value = ''; if (!file) return; setProfileStatus({ type: 'saving', message: 'Uploading background…' }); try { const uploaded = await uploadProfileMedia(file); if (uploaded.kind !== 'image' && uploaded.kind !== 'video') throw new Error('Choose an image or video for Background.'); const nextType = uploaded.kind as 'image' | 'video'; setBackgroundMedia(uploaded.url); setBackgroundMediaType(nextType); writeUserPreferences({ backgroundMedia: uploaded.url, backgroundMediaType: nextType }); const saved = await syncProfile({ background_media: uploaded.url, background_media_type: nextType }); if (!saved) throw new Error('Background uploaded, but profile sync failed. Try Save profile.'); setProfileStatus({ type: 'saved', message: 'Background updated.' }); } catch (error) { setProfileStatus({ type: 'error', message: error instanceof Error ? error.message : 'Background upload failed.' }); } }} />
                    </label>
              ...
              {activeTab === 'connectors' && (
                <div className="space-y-5">
                  <SectionTitle title="Connectors" description="Connect tools to Lunar and show your social links on your public profile." />
                  <ConnectionsSettings />
                  <OAuthConnectors />
                </div>
              )}
