...
export async function GET(request: NextRequest) {
  ...
  if (type === "relationship") { const username = request.nextUrl.searchParams.get("username")?.trim().replace(/^@+/, ""); if (!username || username.toLowerCase() === "uncgpt") return NextResponse.json({ following: false }); const primary = await client.from("user_profiles").select("user_id").eq("username_lower", username.toLowerCase()).maybeSingle(); const target = primary.data || (await client.from("user_profiles").select("user_id").eq("username", username).maybeSingle()).data; if (!target) return NextResponse.json({ following: false }); const { data } = await client.from("profile_follows").select("status").eq("follower_id", userId).eq("following_id", target.user_id).maybeSingle(); return NextResponse.json({ following: Boolean(data) }); }
...
export async function POST(request: NextRequest) {
  ...
  const senderResult = await client.from("user_profiles").select("username").eq("user_id", senderId).maybeSingle(); const targetPrimary = await client.from("user_profiles").select("user_id,username").eq("username_lower", username.toLowerCase()).maybeSingle(); const target = targetPrimary.data || (await client.from("user_profiles").select("user_id,username").eq("username", username).maybeSingle()).data; const sender = senderResult.data;
  if (!target) return NextResponse.json({ error: "Profile not found." }, { status: 404 });
  const senderUsername = sender?.username || session.user.name || "Someone";
  if (action === "follow") { const { error: followError } = await client.from("profile_follows").upsert({ follower_id: senderId, following_id: target.user_id, following_username: target.username, status: "pending" }, { onConflict: "follower_id,following_id" }); if (followError) return NextResponse.json({ error: followError.message }, { status: 500 }); const { data: existingNotification } = await client.from("profile_notifications").select("id").eq("recipient_id", target.user_id).eq("sender_id", senderId).eq("kind", "follow").limit(1).maybeSingle(); if (!existingNotification) { const { error: notificationError } = await client.from("profile_notifications").insert({ recipient_id: target.user_id, sender_id: senderId, sender_username: senderUsername, kind: "follow", body: `@${senderUsername} wants to follow you.` }); if (notificationError) return NextResponse.json({ error: notificationError.message }, { status: 500 }); } return NextResponse.json({ ok: true, request: "pending" }); }
