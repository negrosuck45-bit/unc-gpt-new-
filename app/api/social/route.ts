...
export async function GET(request: NextRequest) {
  ...
  const [{ data: notifications }, { data: messages }, { data: suggestions }] = await Promise.all([client.from("profile_notifications").select("id,sender_username,kind,body,created_at").eq("recipient_id", userId).order("created_at", { ascending: false }).limit(50), client.from("profile_messages").select("id,sender_id,sender_username,thread_username,body,created_at").or(`recipient_id.eq.${userId},sender_id.eq.${userId}`).order("created_at", { ascending: true }).limit(100), client.from("user_profiles").select("username,profile_picture,bio").neq("user_id", userId).neq("username_lower", "uncgpt").limit(8)]);
  return NextResponse.json({ userId, notifications: notifications || [], messages: messages || [], suggestions: suggestions || [] });
}
