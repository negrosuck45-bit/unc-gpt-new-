...
test('renders the reference-style Lunar sign-in card with owned provider choices', () => {
  ...
  assert.match(authPanel, /Continue with GitHub/)
  assert.match(authPanel, /Last used/)
  assert.match(authPanel, /Email address/)
  assert.match(authPanel, /Don&apos;t have an account\?/) 
  assert.match(authPanel, /rounded-\[21px\]/)
...
test('delegates social authentication and passwordless email verification to Clerk', () => {
  ...
  assert.match(authPanel, /strategy: "oauth_github"/)
  ...
  assert.doesNotMatch(authPanel, /Secured by Clerk/)
  assert.doesNotMatch(authPanel, /Development mode/)
